...
const updatePage = async () => {
  ...
  const selected_fruit = fruitsArray.filter(item => item.nutritions.sugar > 5);
  console.log(selected_fruit); 

  filter_names = selected_fruit.map(item =>item.name);


  // TODO: Create a new HTML element to display your data 
  const newElement = document.createElement('cs1300-gallery');

  newElement.innerHTML = filter_names;
